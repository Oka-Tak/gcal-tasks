...
新機能を足す前にこれを読むこと。旧 FastAPI 版はワークツリーから撤去済み（`main` ブランチ =
コミット `0f78751` に復元ポイントとして残っている）。このリポジトリのルートが Kairos 本体。
...
- **ナビゲーションはタブ**（`app/nav.tsx`）: デスクトップは topbar のブラウザ風タブ
  （予定表/ボード/AI/記録、`usePathname` でアクティブ表示）、モバイルは**全ページ常設の下部タブバー**
  （予定表/タスク/ボード/AI/記録）。「戻る」ボタンは廃止。カレンダー内では 予定表/タスク がペイン切替、
  他ページからの「タスク」は `/?pane=tasks`（calendar.tsx が mount 時に読む）。モバイルの「＋予定」は
  右下の FAB。
